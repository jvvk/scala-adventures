...
abstract class TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    def _union(ts: TweetSet, accu:TweetSet): TweetSet = {
      if (ts.isEmpty) accu else _union(ts.tail, if (!accu.contains(ts.head)) accu.incl(ts.head) else accu)
    }
    _union(that,this) 
  }
  ...
  def ascendingByRetweet: Trending = {
    def _ascendingByRetweet(ts: TweetSet, accu: Trending): Trending = {
		if (ts.isEmpty) accu else _ascendingByRetweet(ts.remove(ts.findMin), accu + ts.findMin)
  	}
    _ascendingByRetweet(this, new EmptyTrending) 
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    if (p(head)) tail.filter0(p, accu.incl(head)) else tail.filter0(p, accu)
  }
